...
public class GlobaleExceptionHandler {
  ...
  @ResponseStatus(BAD_REQUEST)
  public Response<CDTPPacket> handleException(DispatchException ex) {
    log.error("无效请求的参数：{}", gson.toJson(ex.getPacket()));
    log.error("无效的请求", ex);
    CDTPPacket cdtpPacket = ex.getPacket();
    Map<String, String> map = new HashMap<>(1);
    map.put("errorMsg", ex.getMessage());
    cdtpPacket.setData(gson.toJson(map).getBytes());
    return Response.failed(BAD_REQUEST, ex.getMessage(), cdtpPacket);
  }
